<?php 
    ...
    $read_investmentinsight = get_read_investmentinsight();
    $paging = get_paging($currPage, count($total_count), $cntPerPage, $currBlock, 'investmentinsight_view_list.php');
    
    function get_total_count()
    {
        global $mysqli, $id;
        $sql = "SELECT * FROM investmentinsight";
        $rows = get_rows($mysqli, $sql);
        
        return $rows;
    }
    function get_read_investmentinsight()
    {
        global $mysqli, $currPage, $cntPerPage;
        $curr = $currPage*$cntPerPage;
        $sql = "SELECT * FROM investmentinsight ORDER BY id desc limit $curr, $cntPerPage";
        $rows = get_rows($mysqli, $sql);
				...
				<div style="width:1000px; min-height:700px; margin:0 auto; padding:40px 0;">
					<div class="pure-css-select-style theme-rounded" style="float:left; width:400px">
						<select id="type" name="type">
							<option value="notice">Investment Insight</option>							
						</select>
					</div>
					...
					<div class="notice_table" style="margin-top:80px;">						
						...
						if(count($total_count) > 0){
						    $i=0;
						foreach ($read_investmentinsight as $items) :
						...
						<div class="notice_row" style="height:50px;">
							...
							<div class="notice_cell text-center">
								<a href="./investmentinsight_view_image_write.php?id=<?=$items['id']?>" target="_blank">
									<?=$items['id']?>
								</a>
							</div>
							<div class="notice_cell">
								<a href="./investmentinsight_view_modify.php?type=<?=$type?>&id=<?=$items['id']?>">
									<?=$list_title ?>
								</a>
							</div>
					...
					<div style="width:100%; text-align:right;">
						<a href="./investmentinsight_view_write.php">글작성</a>
					</div>
